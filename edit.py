...
class Edit(tk.Tk):
    ...
    def __edit(self):
        ...
        if self.word == '':
            tk.messagebox.showerror('Error', 'Please enter a word to edit')
        elif self.new_word == '' and self.new_example == '' and self.new_meaning == '' and self.img_path == None:
            tk.messagebox.showerror('Error', 'Please enter a new word, example, meaning or upload an image')
        else:
            if self.df[self.df.word == self.word].empty == True:
                tk.messagebox.showerror('Error', 'Word not found')
            else:
                if self.new_word != '':
