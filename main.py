...
class App(tk.Tk):
    ...
    def __create_app(self, width=1100, height=800):
        ...
        self.search_btn = self.__create_btn('search', self.__search_word, width-120, 20, font=('Courier', 8))

        self.word_label = self.__create_label(f'Word: {self.word}', 50, height-400)
        self.btn_to_play_word = self.__create_btn('Play', lambda: self.__play_sound(self.word), width-100, height-400, font=('Courier', 8))
        self.example_label = self.__create_label(f'Example: {self.example}', 50, height-300)
        self.btn_to_play_word = self.__create_btn('Play', lambda: self.__play_sound(self.example), width-100, height-300, font=('Courier', 8))
        self.meaning_label = self.__create_label(f'Meaning: {self.meaning}', 50, height-200)
        self.btn_to_play_word = self.__create_btn('Play', lambda: self.__play_sound(self.meaning), width-100, height-200, font=('Courier', 8))
    ...
    def __play_sound(self, text):
        ...
        sound = vlc.MediaPlayer('wave.mp3') #play sound wave
        sound.play()
        time.sleep(1)
        os.remove('wave.mp3')
    #load_img
